	 ...
	 // SİPARİŞLERİN TÜMÜNÜN GÖRÜNDÜĞÜ YER
      ...
      <!-- BAŞLIK -->
       ...
       <div class="row text-left border-bottom-mvc mb-2">  
        	 ...
        	 <div class="col-lg-2 col-xl-2 col-md-12 mb-12 border-left-mvc text-left p-1 mb-2"><h1 class="h3 mb-0 text-gray-800"> <i class="fas fa-th basliktext"></i> SİPARİŞLER </h1></div>
              
 
    <div class="col-lg-3 col-xl-3 col-md-12 mb-12 p-1"><h1 class="h3 mb-0 text-gray-800">Toplam sipariş : <?php echo count($temizsiparisnumaralari); ?></h1></div>  
	 ...
	 Form::Olustur("2",array("type"=>"submit","value"=>"ARA","class"=>"btn btn-sm btn-mvc btn-block mt-1"));		
        ...
        <?php 
	  	...
	  	$veriler=$PanelHarici->joinislemi(
					array("siparisler.kargodurum As kargodurum","siparisler.odemeturu As odemeturu","siparisler.tarih As tarih","uye_panel.ad As uyead",
						"adresler.adres As adres",
						"adresler.id As adresid"
				),
					array("siparisler","uye_panel","adresler"),
					"(siparisler.siparis_no=$value) AND (siparisler.adresid=adresler.id) AND (siparisler.uyeid=uye_panel.id)
					LIMIT 1
					"
				);
...
<div class="col-lg-3 text-dark kalinyap p-2">'.number_format($deger["urunfiyat"],2,",",".").' TL</div>
